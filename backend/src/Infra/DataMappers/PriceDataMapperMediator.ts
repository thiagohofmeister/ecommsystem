...
export class PriceDataMapperMediator extends EntityDataMapperContract<Price, PriceDao> {
  constructor(
  ...
  toDomainEntity(entity: PriceDao): Price {
    ...
    if (entity.variation) {
      price.setVariation(this.variationDataMapper.toDomainEntity(entity.variation))
    }
  ...
  toDaoEntity(domain: Price): PriceDao {
    ...
    if (domain.getVariation()) {
      price.variation = this.variationDataMapper.toDaoEntity(domain.getVariation())
    }
